...
const ChartBuilder: React.FC<ChartBuilderProps> = ({
  ...
  const dataCategories = useMemo(() => getAllDataSeries(), []);
  ...
  const calculatedSeriesData = useMemo((): DataSeries[] => {
  ...
  const chartData = useMemo(() => {
    ...
    return prepareChartData(allActiveSeries, reportRange);
  }, [allActiveSeries, reportRange]);

  const pieData = useMemo(() => {
    if (allActiveSeries.length === 0) return [];
    return preparePieChartData(allActiveSeries, reportRange);
  }, [allActiveSeries, reportRange]);
  ...
  const selectAllInCategory = (category: DataCategory) => {
    setSelectedSeriesIds(prev => {
      const newSet = new Set(prev);
  ...
  const deselectAllInCategory = (category: DataCategory) => {
    setSelectedSeriesIds(prev => {
      const newSet = new Set(prev);
  ...
  const selectAllInSubCategory = (subCategory: SubCategory) => {
    setSelectedSeriesIds(prev => {
      const newSet = new Set(prev);
  ...
  const deselectAllInSubCategory = (subCategory: SubCategory) => {
    setSelectedSeriesIds(prev => {
      const newSet = new Set(prev);
  ...
  const getSelectedCountInCategory = (category: DataCategory): number => {
